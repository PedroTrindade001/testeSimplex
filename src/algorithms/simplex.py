...
class Simplex:
    ...
    def adicionarRestricoes(self, expression: str):
        """Adiciona restrição"""
        delimitador = "<="
        default_format = True

        if not self.validarSimplex(expression):
            raise ValueError("Simplex - Metodo de Duas Fases não implementado!")

        expression = expression.split(delimitador)
        ...
        self.column_b.append(int(expression[1]))
        self.table.append(constraint)

    def validarSimplex(self, constraint: str) -> bool:
